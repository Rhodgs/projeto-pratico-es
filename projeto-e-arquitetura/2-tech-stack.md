...
| :--- | :--- | :--- |
| **Apresentação / Frontend** | Flutter e Node.js | É o modelo base do projeto, onde o aplicativo móvel (Flutter) atua como Cliente realizando requisições e renderizando dashboards reativos, enquanto a API em Node.js atua como Servidor centralizando as regras de negócio. |
| **Mensageria / Notificações** | Node.js, Firebase Cloud Messaging (FCM) e Flutter | Comunicação assíncrona do sistema. O backend gera eventos (Publisher) e o FCM atua como intermediário distribuindo alertas de prazos críticos e lembretes de engajamento para os dispositivos (Assinantes). |
| **Negócio / Backend** | Node.js (TypeScript) | Organização interna da API com divisões claras de responsabilidades (Borda, Serviços e Persistência). O TypeScript adiciona tipagem estática para reduzir erros de execução e proteger as regras pedagógicas. |
| **API / Comunicação** | Node.js (Express) | Estilo arquitetural adotado para a comunicação e tráfego de dados estruturados em formato JSON entre o cliente móvel e o servidor via protocolo HTTP seguro. |
| **Persistência / Dados** | TypeScript, PostgreSQL e Redis | Isola a lógica de persistência de dados. O PostgreSQL protege dados de alta integridade (perfis e troféus). O Redis trata dados em memória para atualização instantânea do Top 50 e inovações sem sobrecarregar o banco principal. |
| **Serviço de Localização** | API do Google Maps | Gateway de integração para geolocalização, permitindo filtrar e mapear unidades de preservação da água e postos de coleta seletiva na ação do estudante. |
| **Autenticação e Segurança** | Autenticação do Firebase | Componente de infraestrutura em nuvem acoplado à arquitetura para garantir controle de acesso seguro e proteção total às credenciais de alunos e professores. |
| **Design e UX** | Figma | Ferramenta de design em nuvem utilizada para projetar a identidade visual e validar os fluxos das telas de gamificação antes de servirem como guia para a organização reativa. |
| **Ambiente de Desenvolvimento** | VS Code | Ambiente central integrado onde a engenharia de software é consolidada, utilizando extensões para depuração ágil tanto do frontend quanto do backend. |
| **Controle de Versão** | GitHub (Online) e GitHub Desktop | O GitHub centraliza o repositório oficial em nuvem, controle de versões e revisões de código (Pull Requests). O GitHub Desktop serve como uma interface gráfica local para agilizar commits e pushes. |
| **Gestão e Documentação** | Notion | Workspace em nuvem que centraliza a documentação técnica do sistema, incluindo levantamento de requisitos, dicionário de dados e regras do motor de gamificação. |
