...
export class TurmaController {
    private readonly turmaService: Pick<TurmaService, 'criarTurma' | 'listarTurmasDoProfessor' | 'excluirTurma'>;
    constructor(turmaService: Pick<TurmaService, 'criarTurma' | 'listarTurmasDoProfessor' | 'excluirTurma'>) {
    ...
    async criarTurma(req: Request, res: Response): Promise<void> {
        ...
        try {
            const turma = await this.turmaService.criarTurma(nome, idProfessor);
            res.status(201).json({
                id: turma.id,
                nome: turma.nome,
                codigo: turma.codigo,
    ...
    // GET /api/turmas
    async listarTurmas(req: Request, res: Response): Promise<void> {
        const { professorId } = req.query;
        const id = typeof professorId === 'string' ? professorId : 'professor-default';
        const turmas = await this.turmaService.listarTurmasDoProfessor(id);
        res.status(200).json(turmas);
    }
    // DELETE /api/turmas/:id
    async excluirTurma(req: Request, res: Response): Promise<void> {
        const { id } = req.params;
        const removida = await this.turmaService.excluirTurma(id as string);
        if (!removida) {
